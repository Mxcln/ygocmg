...
除此之外，程序必须维护独立于任何 `workspace` 的程序级工作区注册表，存放在应用数据目录中，用于 recent workspaces 和“从程序中移除记录”行为：
...
3. 切换当前工作区
4. 删除工作区
5. 展示当前工作区下的所有自定义 `pack`
6. 展示程序已记录的最近工作区
7. 显式打开一个或多个 `pack`
8. 显式关闭已打开的 `pack`
9. 在多个已打开 `pack` 之间切换当前激活 tab
...
1. 必须区分“从程序中移除记录”和“删除磁盘目录”
2. “从程序中移除记录”只删除程序级工作区注册表条目
3. “删除磁盘目录”才真正删除 `workspace` 目录
4. 默认不应影响标准包路径
5. 操作前必须有明确确认
