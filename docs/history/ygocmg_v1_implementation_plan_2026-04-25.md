...
已完成（2026-04-28）。当前实现包含标准 Cards / Strings 只读浏览、索引重建 Job、标准卡号冲突检测、`strings.conf` namespace baseline、资源状态目录预扫描、CDB schema 校验和 stale 状态提示。
...
3. 新建或编辑卡片时可检测标准卡号冲突
4. 标准 Strings 只读浏览可用
5. 标准源文件变化只标记 stale，不自动 rebuild，旧索引仍可浏览
